...
SecHdmi::SecHdmi():
   ...
   // mUIRotVal(0),
    //mG2DUIRotVal(0),
     mUIRotVal(90),
    mG2DUIRotVal(90), //added yqf, for starting display on TV
    mUIFlipVal(0), //added yqf, for flip ops 
    mCurrentHdmiOutputMode(-1),
    mCurrentHdmiResolutionValue(0), // 1080960
    mCurrentHdcpMode(false),
    mCurrentAudioMode(-1),
    mHdmiInfoChange(true),
    mRotateInfoChange(false), //added yqf	
...
bool SecHdmi::connect(void)
{
...
#else
            // add HARDKERNEL
            #if defined(BOARD_USES_HDMI_JUMPER)
            {
                if((resolution = check_default_resolution()) != -1) {
                    
                    if((vout_mode = check_default_vout_mode()) != -1) {
                    	if(vout_mode) 	mHdmiOutputMode = HDMI_OUTPUT_MODE_DVI;
                    ...
                    LOGE("%s::mHdmiResolutionValue(%d) \n"  , __func__, mHdmiResolutionValue);
                    LOGE("%s::mHdmiPresetId(%d) \n"         , __func__, mHdmiPresetId);
                    LOGE("%s::mHdmiStdId(%d) \n"            , __func__, mHdmiStdId);
                    LOGE("%s::mDisplayWidth(%d) \n"         , __func__, mDisplayWidth);
                    LOGE("%s::mDisplayHeight(%d) \n"        , __func__, mDisplayHeight);
                    LOGE("%s::mCompositeStd(%d) \n"         , __func__, mCompositeStd);
                }
            }
            #else
                LOGE("======= %s::Default HDMI Setup value ======= \n", __func__);
                LOGE("%s::HDMI Jumper Not Used \n"      , __func__);
                LOGE("%s::mHdmiOutputMode(%d) \n"       , __func__, mHdmiOutputMode);
                LOGE("%s::mHdmiResolutionValue(%d) \n"  , __func__, mHdmiResolutionValue);
                LOGE("%s::mHdmiPresetId(%d) \n"         , __func__, mHdmiPresetId);
                LOGE("%s::mHdmiStdId(%d) \n"            , __func__, mHdmiStdId);
	...
	//else LOGD("lt::%s::connect:sethdmioutputmode(%d) is ok",__func__,mHdmiOutputMode);//lt,debug
    ...
    if (mHdmiOutputMode >= HDMI_OUTPUT_MODE_YCBCR &&
        ...
        mHdmiInfoChange = true;
        mFlagConnected = true;
        mSecFimc.setRotVal(mUIRotVal); //added yqf, for initial rotation FIMC ->vp display

#if defined(BOARD_USES_EDID)
        // show display..
        //LOGI("lt::%s::connect:go to SecHdmiV4L2Utils.cpp",__func__);//lt,debug
        display_menu();
		...
		if((resolution = check_default_resolution()) != -1) {
			...
			/* 1080P_60 */
			if (mHdmiResolutionValue == 1080960) {
				video.resolution = v1920x1080p_60Hz;
				video.hdmi_3d_format = HDMI_2D_VIDEO_FORMAT;
				LOGI("Enable 1080P HDMI");
			} else if (mHdmiResolutionValue == 720960) {
			/* 720P 60 */
				video.resolution = v1280x720p_60Hz;
				video.hdmi_3d_format = HDMI_2D_VIDEO_FORMAT;
				LOGI("Enable 720P HDMI");
			}
...
bool SecHdmi::disconnect(void)
{
	...
	//yulu modified HDMI_LAYER_GRAPHIC_0  -- > HDMI_LAYER_GRAPHIC_1
    for (int layer = SecHdmi::HDMI_LAYER_BASE + 1; layer <= SecHdmi::HDMI_LAYER_GRAPHIC_1; layer++) {
        if (mFlagHdmiStart[layer] == true && m_stopHdmi(layer) == false) {
...
bool SecHdmi::setUIFlip(unsigned int flipVal, unsigned int hwcLayer)
{
Mutex::Autolock lock(mLock);
 if (mFlagCreate == false) {
        LOGE("%s::Not Yet Created \n", __func__);
  ...
  if(flipVal!=mUIFlipVal)
  	{
  	    mSecFimc.setFlipVal(flipVal);
            mUIFlipVal = flipVal;
           // mHdmiInfoChange = true;
  	}
...
//added yqf, wondering why TV(FIMC3/G2D) recved rotation degree is different from actual LCD used
if(rotVal==270)	
	rotVal=90;
else if(rotVal==90)
	rotVal=270;
    ...
    if (rotVal != mG2DUIRotVal) {
        mG2DUIRotVal = rotVal;
        mHdmiInfoChange = true;
	 mRotateInfoChange=true; //added yqf
    }
    ...
    if (hwcLayer == 0) { /* Rotate in UI only mode */
        if (rotVal != mUIRotVal) {
            ...
            mHdmiInfoChange = true;
	     mRotateInfoChange=true; //added yqf	
	     //LOGI("mUIRotVal:%d,rotVal:%d, mRotateInfoChange:%d\n",mUIRotVal, rotVal,mRotateInfoChange);
	}
...
bool SecHdmi::m_reset(int w, int h, int colorFormat, int hdmiLayer, int hwcLayer)
{
    ...
    if (mHdmiInfoChange == true) {
	 ...
	 if(mRotateInfoChange)  //added yqf
	 	{
	 	 mRotateInfoChange=false;
	        //LOGI("mRotateInfoChange:%d\n",mRotateInfoChange);
	 	}
